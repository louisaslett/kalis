...
AutoTune <- function(t, cache, morgan.dist, gamma = NULL, it = 30, nthreads = 1) {
  ...
  if(class(cache) != "kalisCheckpointTable") {
    stop("cache must be a kalisCheckpointTable.")
  }
...
AutoTuneTarget <- function(bck, cache, t, morgan.dist, Ne, gamma, mu, Pi, e, nthreads) {
  ...
  for(i in length(t):1) {
    ...
    neglog.p.val[i] <- -SAVECALL(obs, as(M, "dsyMatrix"), rep(mu, nrow(M)), rep(sigma, nrow(M)), 25, log = TRUE)[1,4]
  }
