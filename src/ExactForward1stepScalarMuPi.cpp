...
void ExactForward1stepNoExpAVX3_scmuPi_cpp_raw(double *const __restrict__ alpha,
  ...
  for(int_fast32_t recipient=from_rec; recipient<to_rec; ++recipient) {
    ...
    while(l<t) {
      ...
      for(int32_t donor=0; donor<N%(32*8); ++donor) {
        f[recipient-from_rec] += alphaRow[(N/(32*8))*32*8+donor] = 1.0 * (Pi + fratioMulOmRho * alphaRow[(N/(32*8))*32*8+donor]);
      }
...
void ExactForward1stepNoExpAVX3_scmuPi_cpp(NumericMatrix alpha,
                                           ...
                                           const double mu,
                                           NumericVector rho) {
  ExactForward1stepNoExpAVX3_scmuPi_cpp_raw(&(alpha[0]),
                                            &(alpha_f[0]),
                                            &(alpha_f2[0]),
                                            alpha_from_rec,
                                            alpha_t,
...
void ParExactForward1stepNoExpAVX3_scmuPi_cpp(NumericMatrix alpha,
  ...
  for(int_fast32_t i=0; i<nthreads; ++i) {
    threads.push_back(std::thread(ExactForward1stepNoExpAVX3_scmuPi_cpp_raw,
                                  &(alpha[0]),
                                  &(alpha_f[0]),
                                  &(alpha_f2[0]),
                                  alpha_from_rec,
