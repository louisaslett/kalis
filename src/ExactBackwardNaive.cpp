...
void ExactBackwardNaiveC_cpp(NumericMatrix beta,
  ...
  while(l>t) {
    ...
    for(int_fast32_t recipient=from_rec; recipient<to_rec; ++recipient) {
      int_fast32_t recipient_beta = recipient-beta_from_rec;
      int32_t recipient_hap_prev = (hap_locus[l+1][recipient/32] >> recipient%32) & 1;
      int32_t recipient_hap = (hap_locus[l][recipient/32] >> recipient%32) & 1;
      ...
      for(int_fast32_t donor=0; donor<N; ++donor) {
        int32_t donor_hap = (hap_locus[l+1][donor/32] >> donor%32) & 1;
        ...
        donor_hap = (hap_locus[l][donor/32] >> donor%32) & 1;
